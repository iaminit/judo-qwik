...
export const useKataDetail = routeLoader$<KataDetailData>(async ({ params }) => {
  ...
  try {
    ...
    // Find kata by matching slug with name
    const kataRecord = kataRecords.find((kata: any) => {
      const kataSlug = kata.name?.toLowerCase().replace(/\s+/g, '-').replace(/[^a-z0-9-]/g, '');
      return kataSlug === slug;
    });
    ...
    console.log('[KataDetail] Found kata:', kataRecord.name);

    return {
      kata: kataRecord as unknown as Kata,
    };
  } catch (err) {
