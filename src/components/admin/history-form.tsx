...
export default component$<HistoryFormProps>(({ item, isNew, type }) => {
    ...
    const imagePreview = useSignal<string | null>(item?.image ? pbAdmin.files.getUrl(item, item.image) : null);
    ...
    const handleSubmit = $(async (e: Event) => {
        ...
        try {
            const collection = type === 'info' ? 'history' : 'timeline_history';

            if (isNew) {
                await pbAdmin.collection(collection).create(formData);
            } else {
                await pbAdmin.collection(collection).update(item.id, formData);
            }
            nav('/gestione/storia');
        } catch (err: any) {
        ...
        <form onSubmit$={handleSubmit} class="bg-white dark:bg-gray-900 rounded-3xl p-8 border border-gray-100 dark:border-gray-800 shadow-sm space-y-6">
            ...
            <div class="grid grid-cols-1 gap-6">
                ...
                {type === 'info' && (
                    <div class="space-y-6">
                        <div class="space-y-2">
                            <label class="block text-xs font-black text-gray-400 uppercase tracking-widest px-1">Immagine (Opzionale)</label>
                            <input
                                type="file"
                                name="image"
                                accept="image/*"
