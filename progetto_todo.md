...
- [x] **Task amministrativi** con email Mailgun ✅ (AGGIORNATO A SCHEMA ITALIANO)
...
## 🔄 MIGRAZIONE SCHEMA DATABASE (COMPLETATA ✅)

### admin_tasks → task_admin (Schema Italiano)

**Data completamento**: 31 Dicembre 2025

#### File Creati
- ✅ `pb_schema_task_admin.json` - Schema PocketBase italiano
- ✅ `migrate_admin_tasks.mjs` - Script migrazione automatica
- ✅ Componente `admin-task-list.tsx` aggiornato (campi italiani + FK utenti)
- ✅ API `send-task-reminder` aggiornata (supporto bilingue IT/EN)

#### Cambiamenti Schema

| Prima (EN) | Dopo (IT) | Tipo | Note |
|-----------|-----------|------|------|
| `title` | `titolo` | text | Rinominato |
| `description` | `contenuto` | editor | HTML supportato |
| `completed` | `completato` | bool | Rinominato |
| `priority` | `priorita` | select | bassa/media/alta/urgente |
| `assigned_to` | `assegnato_a_id` | **relation → users** | FK invece di stringa! |
| `due_date` | `data_riferimento` | date | Rinominato |
| - | `stato` | select | NUOVO: aperto/in_corso/bloccato/completato |
| - | `in_evidenza` | bool | NUOVO: pin task urgenti |
| - | `promemoria_inviato` | bool | NUOVO: tracking email |
| - | `tags` | text | NUOVO: per ricerca globale |

#### Nuove Funzionalità
- ✅ Mostra nome utente reale (via expand FK)
- ✅ Badge stato workflow (4 stati vs 2)
- ✅ Indicatore 📌 per task in evidenza
- ✅ Tracking promemoria anti-spam
- ✅ Ricerca globale compatibile (titolo, tags, contenuto)

#### Come Eseguire Migrazione
```bash
# 1. Backup
cp -r pb_data pb_data_backup

# 2. Importa schema (PocketBase UI)
# http://127.0.0.1:8090/_/ → Collections → Import → pb_schema_task_admin.json

# 3. Migra dati
node migrate_admin_tasks.mjs
# (interattivo: chiede email admin, password, conferma)

# 4. (Opzionale) Elimina vecchia collection admin_tasks
```

**Riferimento completo**: Vedi `reorg.md` sezione 2.12 e 14
...
- [ ] Test email task reminder (Mailgun)
...
### Script Utili
- ✅ `migrate_admin_tasks.mjs` - Migrazione task (già eseguibile)
- ✅ `pb_schema_task_admin.json` - Schema PocketBase task_admin
- ✅ `check_*.mjs` (18 script) - Debug e ispezione PocketBase
